...
begin
  ...
  with so as (select o.id order_id, o.campaign_id, o.turn_id, o.campaign_player_id, t.id territory_id, roll.dice_result, roll.dice_result >= 3 success from public.orders o join public.territories t on t.id = o.target_territory_id cross join lateral (select (floor(random() * 6)::int + 1) dice_result where o.id is not null) roll where o.campaign_id = v_campaign.id and o.turn_id = v_turn.id and o.status = 'submitted' and o.action_type = 'conquer' and t.owner_campaign_player_id is null and not exists (select 1 from public.orders rival where rival.campaign_id = o.campaign_id and rival.turn_id = o.turn_id and rival.status = 'submitted' and rival.action_type = 'conquer' and rival.target_territory_id = o.target_territory_id and rival.id <> o.id)),
