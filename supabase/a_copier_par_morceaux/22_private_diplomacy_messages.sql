alter table public.campaign_messages
  add column if not exists recipient_campaign_player_id uuid;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'campaign_messages_recipient_campaign_player_fk'
      and conrelid = 'public.campaign_messages'::regclass
  ) then
    alter table public.campaign_messages
      add constraint campaign_messages_recipient_campaign_player_fk
...
with check (
  ...
  and exists (
    select 1 from public.campaign_players author
    where author.id = campaign_player_id
      and author.campaign_id = campaign_messages.campaign_id
      and author.user_id = auth.uid()
      and author.status = 'active'
  )
  and exists (
    select 1 from public.campaign_players recipient
    where recipient.id = recipient_campaign_player_id
