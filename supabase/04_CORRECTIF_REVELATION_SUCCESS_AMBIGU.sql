...
declare
  ...
  v_income_player_count int := 0; v_income_glory_total int := 0;
begin
  ...
  if exists (select 1 from public.orders o left join public.territories source on source.id = o.source_territory_id left join public.territories target on target.id = o.target_territory_id left join public.territory_adjacencies adjacency on adjacency.campaign_id = o.campaign_id and adjacency.territory_code = source.code and adjacency.adjacent_territory_code = target.code where o.campaign_id = v_campaign.id and o.turn_id = v_turn.id and o.status = 'submitted' and ((o.action_type = 'conquer' and (source.id is null or target.id is null or source.owner_campaign_player_id is distinct from o.campaign_player_id or adjacency.id is null or target.owner_campaign_player_id = o.campaign_player_id)) or (o.action_type = 'fortify' and (target.id is null or target.owner_campaign_player_id is distinct from o.campaign_player_id or target.type = 'fort' or target.is_fortified)))) then
  ...
  with ib as (
    ...
    select o.campaign_id, o.turn_id, o.id, t.id, o.campaign_player_id, t.owner_campaign_player_id, v_turn.army_base_points, nullif(concat_ws(' ', case when t.type = 'fort' then 'Forteresse : défenseur +200 points d''armée.' when t.is_fortified then 'Fortification : défenseur +200 points d''armée.' else null end, case when t.type = 'magic_tower' then 'Tour magique : défenseur dispose d''un magicien niveau 1 (santé 8, sauvegarde 4+).' else null end), '')
  ...
  with so as (select o.id order_id, o.campaign_id, o.turn_id, o.campaign_player_id, t.id territory_id, t.type territory_type, t.special_reward_claimed_at is null special_reward_available, v_turn.turn_number <= 3 opening_auto_conquest, case when v_turn.turn_number <= 3 then 6 when support.adjacent_support_count >= 3 then 6 else roll.dice_result end dice_result, support.adjacent_support_count, case when v_turn.turn_number <= 3 then null when support.adjacent_support_count >= 2 then 2 when t.type in ('dragon', 'giant') then 4 else 3 end conquest_threshold, v_turn.turn_number <= 3 or support.adjacent_support_count >= 3 or roll.dice_result >= case when support.adjacent_support_count >= 2 then 2 when t.type in ('dragon', 'giant') then 4 else 3 end exploration_success from public.orders o join public.territories t on t.id = o.target_territory_id cross join lateral (select count(distinct owned.id)::int adjacent_support_count from public.territory_adjacencies adjacency join public.territories owned on owned.campaign_id = o.campaign_id and owned.code = adjacency.adjacent_territory_code and owned.owner_campaign_player_id = o.campaign_player_id where adjacency.campaign_id = o.campaign_id and adjacency.territory_code = t.code) support cross join lateral (select (floor(random() * 6)::int + 1) dice_result where o.id is not null) roll where o.campaign_id = v_campaign.id and o.turn_id = v_turn.id and o.status = 'submitted' and o.action_type = 'conquer' and t.owner_campaign_player_id is null and not exists (select 1 from public.orders rival where rival.campaign_id = o.campaign_id and rival.turn_id = o.turn_id and rival.status = 'submitted' and rival.action_type = 'conquer' and rival.target_territory_id = o.target_territory_id and rival.id <> o.id)),
  ...
  gu as (update public.campaign_players cp set glory = glory + bonus.glory_gain, updated_at = now() from (select ie.campaign_player_id, count(*)::int + (count(*) filter (where ie.exploration_success and so.territory_type = 'ruins' and so.special_reward_available))::int + ((count(*) filter (where ie.exploration_success and so.territory_type in ('dragon', 'giant')))::int * 3) glory_gain from ie join so on so.order_id = ie.order_id group by ie.campaign_player_id) bonus where cp.id = bonus.campaign_player_id returning cp.id),
  tu as (update public.territories t set owner_campaign_player_id = ie.campaign_player_id, special_reward_claimed_at = case when so.territory_type = 'ruins' and so.special_reward_available then now() else t.special_reward_claimed_at end, updated_at = now() from ie join so on so.order_id = ie.order_id where t.id = ie.territory_id and ie.exploration_success returning t.id)
  insert into public.campaign_logs (campaign_id, turn_id, type, title, description, created_by_user_id)
  select ie.campaign_id, ie.turn_id, 'exploration_result', case when ie.exploration_success then 'Conquête réussie' else 'Conquête échouée' end, cp.display_name || ' tente de conquérir ' || t.code || ' - ' || t.name || ' : ' || case when ie.exploration_success then 'réussite' else 'échec' end || case when so.opening_auto_conquest then ' automatique pendant les 3 premiers tours' when so.adjacent_support_count >= 3 then ' automatique grâce à ' || so.adjacent_support_count || ' territoires adjacents contrôlés' else ' sur un D6 automatique de ' || ie.dice_result || ' (réussite sur ' || so.conquest_threshold || '+, soutien adjacent : ' || so.adjacent_support_count || ')' end || '. +1 Gloire' || case when ie.exploration_success and so.territory_type = 'ruins' and so.special_reward_available then ', +1 Gloire de ruines' else '' end || case when ie.exploration_success and so.territory_type = 'dragon' then ', +3 Gloire de Dragon' when ie.exploration_success and so.territory_type = 'giant' then ', +3 Gloire de Géant' else '' end || '.', auth.uid() from ie join so on so.order_id = ie.order_id join public.campaign_players cp on cp.id = ie.campaign_player_id join public.territories t on t.id = ie.territory_id;
  v_battle_count := v_enemy_battle_count + v_contested_battle_count;
  update public.territories target set is_fortified = true, updated_at = now() from public.orders o where target.id = o.target_territory_id and o.campaign_id = v_campaign.id and o.turn_id = v_turn.id and o.status = 'submitted' and o.action_type = 'fortify' and target.type <> 'fort' and not target.is_fortified;
  ...
  if v_battle_count = 0 then
    ...
    insert into public.campaign_logs (campaign_id, turn_id, type, title, description, created_by_user_id)
    values (v_campaign.id, v_turn.id, 'turn_finished', 'Tour terminé automatiquement', 'Aucune bataille à résoudre. Le tour ' || v_campaign.current_turn_number || ' est terminé automatiquement. Le tour ' || v_next_turn_number || ' commence avec ' || v_next_army_base_points || ' points d''armée.' || case when v_income_glory_total > 0 then ' Revenus de fin de tour : ' || v_income_glory_total || ' Gloire attribuée à ' || v_income_player_count || ' joueur(s).' else '' end, auth.uid());
  else
