...
begin
  ...
  select ie.campaign_id, ie.turn_id, 'exploration_result', case when ie.exploration_success then 'Conquête réussie' else 'Conquête échouée' end, cp.display_name || ' tente de conquérir ' || t.code || ' - ' || t.name || ' : ' || case when ie.exploration_success then 'réussite' else 'échec' end || ' sur un D6 automatique de ' || ie.dice_result || '. +1 Gloire.', auth.uid() from ie join public.campaign_players cp on cp.id = ie.campaign_player_id join public.territories t on t.id = ie.territory_id;
