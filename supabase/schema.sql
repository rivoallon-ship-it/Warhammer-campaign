...
create index if not exists campaign_messages_campaign_id_created_at_idx on public.campaign_messages(campaign_id, created_at desc);
create index if not exists campaign_messages_recipient_created_at_idx on public.campaign_messages(campaign_id, recipient_campaign_player_id, created_at desc);
...
drop policy if exists "Messages readable by active campaign members" on public.campaign_messages;
drop policy if exists "Diplomacy messages readable by participants" on public.campaign_messages;
...
using (
  exists (
    ...
    where cp.campaign_id = campaign_messages.campaign_id
      ...
      and cp.status = 'active'
      and cp.id in (
        campaign_messages.campaign_player_id,
        campaign_messages.recipient_campaign_player_id
      )
  )
);

drop policy if exists "Active players insert their own messages" on public.campaign_messages;
drop policy if exists "Active players insert their own diplomacy messages" on public.campaign_messages;
...
with check (
  ...
  and exists (
    select 1
    from public.campaign_players author
    where author.id = campaign_messages.campaign_player_id
      and author.campaign_id = campaign_messages.campaign_id
      and author.user_id = auth.uid()
      and author.status = 'active'
  )
  and exists (
    select 1
    from public.campaign_players recipient
    where recipient.id = campaign_messages.recipient_campaign_player_id
      and recipient.campaign_id = campaign_messages.campaign_id
