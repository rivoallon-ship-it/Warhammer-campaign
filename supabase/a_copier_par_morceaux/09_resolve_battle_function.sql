...
begin
  ...
  if exists (select 1 from public.battle_participants where battle_id = v_battle.id) then
    update public.campaign_players cp set glory = glory + case when cp.id = submitted_winner_campaign_player_id and v_winner_role = 'defender' then 2 when cp.id = submitted_winner_campaign_player_id then 3 else 1 end, updated_at = now()
    where exists (select 1 from public.battle_participants bp where bp.battle_id = v_battle.id and bp.campaign_player_id = cp.id);
  else
    update public.campaign_players set glory = glory + case when id = submitted_winner_campaign_player_id and v_winner_role = 'defender' then 2 when id = submitted_winner_campaign_player_id then 3 else 1 end, updated_at = now()
    where id in (v_battle.attacker_campaign_player_id, v_battle.defender_campaign_player_id);
  end if;
  update public.territories set owner_campaign_player_id = submitted_winner_campaign_player_id, is_fortified = case when v_battle.defender_bonus is not null then false else is_fortified end, updated_at = now() where id = v_battle.territory_id;
  ...
  values (v_battle.campaign_id, v_battle.turn_id, 'battle_result', 'Bataille résolue',
    v_winner.display_name || ' remporte la bataille pour ' || v_territory.code || ' - ' || v_territory.name || '. +' || case when v_winner_role = 'defender' then 2 else 3 end || ' Gloire vainqueur' || case when v_loser_count > 0 then ', +1 Gloire pour chaque autre participant.' else '.' end
